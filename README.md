...
| `FEISHU_AI_ID`           | Feishu app ID paired with `FEISHU_AI_SECRET` to obtain a tenant access token. Required for WeChat imports.                  |
| `FEISHU_AI_SECRET`       | Secret for the Feishu app identified by `FEISHU_AI_ID`. Required for WeChat imports.                                        |
| `FEISHU_AILY_APP_ID`     | Aily application ID passed when creating runs for WeChat article sanitization and translation. Required for WeChat imports. |
